...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 transform = glm::mat4(1.0f);// Initialize to the identity matrix.(手动初始化为单位矩阵）
        // 逆时针旋转90°，缩放到原来的0.5倍
        //transform = glm::rotate(transform,glm::radians(90.0f),glm::vec3(0.0,0.0,1.0));
        //transform = glm::scale(transform,glm::vec3(0.5,0.5,0.5));

        // 先旋转，再位移（记住：实际变换顺序与阅读顺序相反）
        transform = glm::translate(transform,glm::vec3(0.5f,-0.5f,0.0f));
        transform = glm::rotate(transform,(float)glfwGetTime(),glm::vec3(0.0f,0.0f,1.0f));

        // Exercise 1:交换上面的次序，会出现偏移
//        transform = glm::rotate(transform,(float)glfwGetTime(),glm::vec3(0.0f,0.0f,1.0f));
//        transform = glm::translate(transform,glm::vec3(0.5f,-0.5f,0.0f));
        ...
        // 简单理解：传递一个Matrix4fv（4阶矩阵）的东西给Uniform
        // 第一个参数：uniform的位置值
        // 第二个参数：要发送多少个矩阵
        // 第三个参数：是否要对矩阵进行置换，glm为列主序，与OpenGL相同，不用置换
        // 第四个参数：矩阵的真正数据，需要先用value_ptr进行变换
        ...
        // Exercise 2: Second transformation
        // 方法：先draw了第一个箱子，再重置transform之后用DrawElement画出第二个箱子
        //-----------
        transform = glm::mat4(1.0f);// reset to an identity matrix
        transform = glm::translate(transform,glm::vec3(-0.5f,0.5f,0.0f));
