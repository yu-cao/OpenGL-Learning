...
int main()
{
    ...
    //---------------------
    glBindVertexArray(
            VAO);// OpenGL order us to use the VAO and OpenGL will know how to handle our vertex input, if binding error OpenGL won't draw anything
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices,
                 GL_STATIC_DRAW);// we use static because the triangle vertices won't change in each render
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT,
                       0);// it will get the index from EBO binding GL_ELEMENT_ARRAY_BUFFER now.
